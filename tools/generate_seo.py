...
_LLMS_NOTES = {
    "maive":
        ...
        "for the other two: Irsova, Bom, Havranek and Rachinger, Nature Communications 16, 8454 "
        "(2025), doi 10.1038/s41467-025-63261-0; R package on CRAN at "
        "https://cran.r-project.org/package=MAIVE; runs in a browser at https://www.easymeta.org/. "
        "MAIVE instruments the reported standard error with sample size. "
        "Two methods build on it and NEITHER HAS A PAPER YET. "
        "WAIVE, the Weighted Adjustment Instrumental Variable Estimator, is the MORE AGGRESSIVE "
        "correction: after MAIVE's instrument it downweights the excess precision that remains, by "
        "an exponential tilt that bites only on negative residuals and hardest where the "
        "t-statistic is near 1.96. "
        "RDT, the residual discontinuity test, is a TEST FOR P-HACKING built on the same first "
        "stage, not an estimator. Excess precision is the residual from regressing the log "
        "standard error on log sample size, negative where the standard error is smaller than "
        "sample size predicts; RDT asks whether the mean of that residual jumps at the "
        "significance threshold, with log absolute t as the running variable and log 1.96 as the "
        "cutoff, fitted locally on each side, and reported with a falsification battery (donut "
        "hole, placebo cutoffs, density tests, bandwidth sensitivity, placebo outcome). "
        "BOTH are offered in EasyMeta, each labelled Experimental in its model-type list, beside "
        "MAIVE, WLS and RTMA. "
        "The only sources for either are three conference presentations: "
        "https://meta-analysis.cz/waive_ottawa.pdf (MAER-Net, University of Ottawa, October 2025, "
        "26 slides, on WAIVE), https://meta-analysis.cz/waive_chania.pdf (SRSM 2026, Chania, 12 "
        "slides plus a nine-slide technical appendix), and "
        "https://meta-analysis.cz/rdt_chemnitz.pdf (MAER-Net Colloquium 2026, Chemnitz, 14 slides "
        "plus a ten-slide technical appendix, titled The Residual Discontinuity Test for "
        "p-Hacking and the fullest treatment of RDT). Cite them as presentations, not as "
        "published results. An earlier MAIVE-only talk, without WAIVE or RDT, is at "
        "https://meta-analysis.cz/maive/maive_slides.pdf (MAER-Net, University of Augsburg, "
        "September 2024, 11 slides); the later decks cover the same ground and add the "
