"""Build the homepage answer board and fold the result sentences into a <details>.
...
The full sentences are NOT removed. They keep their existing markup, byte for byte, inside a
...
def main():
    ...
    for field, cls in FIELDS:
        ...
        for r in group:
            ...
            # A paper filed under another project's landing lives at its estimates.csv
            # url, not at /<project>/ (the 2020 reporting guidelines under /guidelines/).
            href = re.sub(r"^https?://meta-analysis\.cz", "", r["url"].rstrip("/")) + "/" \
                if r.get("url") else f"/{p}/"
            out.append(f'  <li{li}><a href="{href}">')
    ...
    if refresh:
        # The sentences inside the <details> are generated by build_estimates.py into
