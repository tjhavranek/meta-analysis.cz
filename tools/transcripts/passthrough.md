...
where $\Delta\mathrm{bank\ rate}^k_{i,t} = \mathrm{bank\ rate}^k_{i,t} - \mathrm{bank\ rate}^k_{i,t-1}$ stands for the change in bank $i$'s rate on product $k$ between months $t-1$ and $t$ (due to data limitations we use a maximum of one lag in all estimations of the pass-through mechanism), $\Delta\mathrm{market\ rate}^k_t$ is the change in the corresponding financial market interest rate in period $t$ for product $k$, $\alpha^k$ measures the short-term pass-through of the market rate to bank $i$'s rate for product $k$, $\Delta\mathrm{bank\ rate}^k_{i,t-1}$ is the change in the bank rate in the previous month, $\beta^k_i$ captures persistence in bank rate changes, $\delta^k_i$ denotes the long-term equilibrium pass-through coefficient, $\mu^k_i$ is the mean markup (spread) over the market rate, $\gamma^k_i$ denotes the speed of adjustment, and $\varepsilon^k_{i,t}$ is a disturbance term. The mean adjustment lag at which the market rates are fully passed through to the bank rates can be computed as (δ − α)/γ (Hendry, 1995).
...
where $SE_{ijk}$ denotes the standard error of the estimate of the adjustment lag for bank i, period j, and product k (the standard error is approximate and estimated using the delta method). Similarly to the previous case we include the corresponding pass-through coefficient for deposits.
...
FIGURE 2. Selected Determinants of Pass-Through to Loan Rates.
...
FIGURE B1. Development of the Estimated Frontier Efficiencies During 2004–2013.
Notes: The box shows interquartile range (P25–P75) with median highlighted. Whiskers cover the interval from (P25−1.5· interquartile range) to (P75+1.5· interquartile range) if such estimates exist. The dots show the remaining (outlying) estimates.
