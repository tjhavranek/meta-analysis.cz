...
$$ \Delta\mathrm{bank\ rate}^k_{i,t} = \alpha^k_i \Delta\mathrm{market\ rate}^k_t + \beta^k_i \Delta\mathrm{bank\ rate}^k_{i,t-1} + \gamma^k_i (\mathrm{bank\ rate}^k_{i,t-1} - \delta^k_i \mathrm{market\ rate}_{t-1} - \mu^k_i) + \varepsilon_{i,t} $$ (1)
...
$$ \Delta\mathrm{bank\ rate}^k_{i,t} = \alpha^k_i \Delta\mathrm{market\ rate}^k_t + \beta^k_i \Delta\mathrm{bank\ rate}^k_{i,t-1} + \gamma^k_i (\mathrm{bank\ rate}^k_{i,t-1} - \delta^k \mathrm{market\ rate}_{t-1} - \mu^k) + \varepsilon_{i,t} $$ (2)
...
The mean adjustment lag, on the other hand, seems to be often similar for loan and deposit products. Values of the adjustment lag for loan products larger than four months only occur for banks that also display slow adjustment of deposit rates to changes in the market rate. While there are some
...
Our findings concerning the impact of the financial crisis contrast with the results of Illes and Lombardi (2013),^{9} who find little change in the interest rate pass-through for major economies. Hristov et al. (2014), on the other hand, using fresher data, show that the pass-through has become significantly distorted in the euro area since 2008, which is consistent with our results for the same period and Czech data. Hansen and Welz (2011) examine Swedish data and report results in between those of Illes and Lombardi (2013) and Hristov et al. (2014): in Sweden the crisis has not affected interest rate pass-through for products with short maturities, but has distorted pass-through to lending rates at longer maturities.
...
Hansen, N.-J. H. and P. Welz (2011): “Interest Rate Pass-through During the Global Financial Crisis: The Case of Sweden.” OECD Economics Department Working Papers 855, OECD Publishing. Bank Efficiency and Interest Rate Pass-Through: Evidence from Czech Loan Products 25
...
26 Tomáš Havránek, Zuzana Iršová, and Jitka Lešanovská
...
ALT B1. Box plots of the estimated frontier efficiency scores for each year from 2004 to 2013, one box per year for the SFA scores and one for the DEA scores, with medians, interquartile ranges, whiskers and outlying points.
...
ALT B2. Two kernel density curves of the estimated efficiency scores between 0 and 1, one for the SFA scores and one for the DEA scores; the SFA density is concentrated at high scores while the DEA density is flatter and reaches further down the scale.
