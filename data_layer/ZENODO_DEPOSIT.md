...
The folder `meta-analysis-cz-harmonised-v1.0.0/` — **50 files, 14.9 MB**. Rebuild it any
time with `python data_layer/12_zenodo_bundle.py`.

| File | What |
|---|---|
| `estimates_harmonised.csv` / `.parquet` | 54,087 estimates, 39 literatures, 41 columns |
...
2. **New upload**, then drag in the contents of `meta-analysis-cz-harmonised-v1.0.0/`.
...
6. Send the DOI back and it goes into `CITATION.cff`, `datasets.json` and the `/datasets/`
   page.
...
The harmonised table is `0.9.0-beta`, and 20 of its 39 literatures rest on arithmetic
effect/standard-error pairing rather than a domain review. That is stated in `README.md`,
